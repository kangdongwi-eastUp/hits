    ...
    <main class="minWidth3">
        ...
        <div class="wrap_contents">
            ...
            <h4>
                {{ $board->name }}
                <br>
                {{ preg_replace("/(^02.{0}|^01.{1}|[0-9]{3})([0-9]+)([0-9]{4})/", "$1-$2-$3", $board->phone) }}
            </h4>

            <h3>신청인이 업체이시면 업체명을 작성해주세요.</h3>
            <h4>
                {{ $board->option1 === 'y' ? '셀프 직영 공사 입니다.' : $board->company_name }}
            </h4>

            <h3>현장 담당자명과 연락처를 작성해주세요.</h3>
            <h4>
                {{ $board->manager_name }}<br>{{ $board->manager_phone }}
            </h4>

            <h3>입주자의 성명과 연락처를 작성해주세요.</h3>
            <h4>
                {{ $board->resident_name }}
                <br>
                {{ preg_replace("/(^02.{0}|^01.{1}|[0-9]{3})([0-9]+)([0-9]{4})/", "$1-$2-$3", $board->resident_phone) }}
            </h4>
